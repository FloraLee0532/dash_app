<template>
    <div class="fixed bottom-7.5 right-7.5 text-xl z-990 shadow-soft-lg rounded-circle cursor-pointer bg-white px-4 py-2 text-slate-700" @click="$emit('toggle')">
      <i class="py-2 pointer-events-none fa fa-cog"></i>
    </div>
    <div :class="`z-sticky shadow-soft-3xl w-90 ease-soft fixed top-0 left-auto flex h-full min-w-0 flex-col break-words rounded-none border-0 bg-white bg-clip-border px-2.5 duration-200 ${isOpen ? '0' : '-right-90'}`">
      <div class="px-6 pt-4 pb-0 mb-0 bg-white border-b-0 rounded-t-2xl">
        ...
        <div class="float-right mt-6">
          <button @click="$emit('close')" class="inline-block p-0 mb-4 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer hover:scale-102 leading-pro text-xs ease-soft-in tracking-tight-soft bg-150 bg-x-25 active:opacity-85 text-slate-700">
      ...
      <div class="flex-auto p-6 pt-0 sm:pt-4">
        ...
        <div class="my-2 text-left">
          <span v-for="(color, index) in sidebarColors" :key="index" 
                :class="`text-xs rounded-circle h-5.75 mr-1.25 w-5.75 ease-soft-in-out bg-gradient-to-tl ${color.gradient} relative inline-block cursor-pointer whitespace-nowrap border border-solid ${activeColor === index ? 'border-slate-700' : 'border-white'} text-center align-baseline font-bold uppercase leading-none text-white transition-all duration-200 hover:border-slate-700`"
                @click="activeColor = index"></span>
        </div>
        ...
        <div class="flex">
          <button @click="sidenavType = 'transparent'" 
                  :class="`inline-block w-full px-4 py-3 mb-2 font-bold text-center uppercase align-middle transition-all border border-transparent border-solid rounded-lg cursor-pointer xl-max:cursor-not-allowed xl-max:opacity-65 xl-max:pointer-events-none xl-max:bg-gradient-to-tl xl-max:from-purple-700 xl-max:to-pink-500 xl-max:text-white xl-max:border-0 hover:scale-102 hover:shadow-soft-xs active:opacity-85 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 ${sidenavType === 'transparent' ? 'bg-gradient-to-tl from-purple-700 to-pink-500 bg-fuchsia-500 text-white hover:border-fuchsia-500' : 'border-fuchsia-500 bg-none text-fuchsia-500 hover:border-fuchsia-500'}`">Transparent</button>
          <button @click="sidenavType = 'white'" 
                  :class="`inline-block w-full px-4 py-3 mb-2 ml-2 font-bold text-center uppercase align-middle transition-all bg-transparent border border-solid rounded-lg cursor-pointer xl-max:cursor-not-allowed xl-max:opacity-65 xl-max:pointer-events-none xl-max:bg-gradient-to-tl xl-max:from-purple-700 xl-max:to-pink-500 xl-max:text-white xl-max:border-0 hover:scale-102 hover:shadow-soft-xs active:opacity-85 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 ${sidenavType === 'white' ? 'bg-gradient-to-tl from-purple-700 to-pink-500 bg-fuchsia-500 text-white hover:border-fuchsia-500' : 'border-fuchsia-500 bg-none text-fuchsia-500 hover:border-fuchsia-500'}`">White</button>
        </div>
  ...
  export default {
    ...
    setup() {
      ...
      const sidenavType = ref('transparent')
