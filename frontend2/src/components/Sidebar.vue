<template>
  <aside :class="`max-w-62.5 ease-nav-brand z-990 fixed inset-y-0 my-4 ml-4 block w-full flex-wrap items-center justify-between overflow-y-auto rounded-2xl border-0 bg-white p-0 antialiased shadow-none transition-transform duration-200 ${isOpen ? 'xl:left-0 xl:translate-x-0' : '-translate-x-full'} xl:bg-transparent`">
    <div class="h-19.5">
      <i @click="$emit('close')" class="absolute top-0 right-0 p-4 opacity-50 cursor-pointer fas fa-times text-slate-400 xl:hidden"></i>
...
export default defineComponent({
  ...
  setup() {
    ...
    const accountItems = [
      ...
      {
        title: 'Sign In',
        icon: `<svg width="12px" height="12px" viewBox="0 0 40 44" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      ...
      {
        title: 'Sign Up',
        icon: `<svg width="12px" height="20px" viewBox="0 0 40 40" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
